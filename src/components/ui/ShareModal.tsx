...
interface Props {
  ...
  subtitle?: string
  /** Заголовок для системного «Поделиться» / мессенджеров */
  shareTitle: string
  /** Текст-описание (без ссылки) — к нему добавляется url */
  shareText: string
}

/** Единая форма «Поделиться» — ссылка, мессенджеры, системный share.
 *  Используется для счёта, запчасти и др. (тексты задаются через пропсы). */
export default function ShareModal({ isOpen, onClose, url, title, subtitle, shareTitle, shareText }: Props) {
  ...
  const messengers = [
    ...
    { label: 'Telegram', href: `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(shareTitle)}`, cls: 'bg-blue-50 text-blue-700 hover:bg-blue-100', Icon: Send },
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <input readOnly value={url} className="form-input text-xs flex-1" onFocus={e => e.target.select()} />
