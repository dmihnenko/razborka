...
export default function ExchangeRateWidget() {
  ...
  const fetchNow = async () => {
    try {
      ...
      toast.success(t('exchangeRate.toastUpdated', { rate: r.toFixed(2) }))
    } catch {
      toast.error(t('exchangeRate.toastFetchError'))
    }
  }

  const saveManual = () => {
    const v = parseFloat(val.replace(',', '.'))
    if (!v || v <= 0) { toast.error(t('exchangeRate.toastInvalid')); return }
    setManualRate(v)
    toast.success(t('exchangeRate.toastSaved'))
  }
  ...
  return (
    <div className="cab-card p-3 flex items-center gap-3" style={{ borderColor: 'var(--cab-signal)' }}>
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-bold leading-tight" style={{ color: 'var(--cab-ink)' }}>{t('exchangeRate.title')}</p>
        <p className="text-xs mt-0.5" style={{ color: 'var(--cab-ink-2)' }}>
          {t('exchangeRate.subtitle', { rate: rate.toFixed(2) })}
        </p>
      </div>
      ...
      {editing ? (
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          />
          <button onClick={saveManual} className="cab-btn cab-btn-primary cab-btn-sm" title={t('exchangeRate.save')}>
            <Check className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => { setEditing(false); setVal('') }} className="cab-btn cab-btn-ghost cab-btn-sm" title={t('exchangeRate.cancel')}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={fetchNow} disabled={fetching} className="cab-btn cab-btn-signal cab-btn-sm">
            ...
            <span className="hidden sm:inline">{t('exchangeRate.update')}</span>
          </button>
          <button onClick={() => { setEditing(true); setVal(rate.toFixed(2)) }} title={t('exchangeRate.manualEntry')}
