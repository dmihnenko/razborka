    ...
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          {/* Утверждённые / отклонённые — таблица */}
          {others.length > 0 && (
            <div className="card p-0 overflow-hidden">
              <table className="w-full">
                <thead>
                  <tr>
                    <th className="table-header-cell">Марка</th>
                ...
                <tbody className="grid-hairline">
                  {others.map((r: CarModelRow) => {
                    const badge = STATUS_BADGE[r.status]
                    return (
                      <tr key={r.id} className="table-row">
                        <td className="table-cell font-semibold text-gray-900">{r.make}</td>
                        <td className="table-cell text-gray-700">{r.model}</td>
