...
// Приветственное (оно же тестовое) сообщение при подключении: что подключилось,
// что будет приходить и где этим управлять в приложении.
const WELCOME_MESSAGE =
  '👋 <b>Привет! Уведомления разборки подключены.</b>\n' +
  'Это тестовое сообщение — связь с ботом работает. ✅\n' +
  '\n' +
  '<b>Что будет приходить сюда:</b>\n' +
  '🛒 новые заявки с маркета (телефон, имя, сумма)\n' +
  '🆕 новые заказы клиентов\n' +
  '⏳ напоминание, когда подписка заканчивается (за 3 дня)\n' +
  '\n' +
  '⚙️ Управлять подключением можно в приложении: «Настройки» → «Telegram-уведомления».'
  ...
  if (chatId && text.startsWith('/start')) {
    ...
    if (UUID_RE.test(arg)) {
      ...
      if (error) {
        ...
        await tgSend(chatId, WELCOME_MESSAGE)
      }
    } else {
