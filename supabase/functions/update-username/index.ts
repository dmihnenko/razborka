...
serve(async (req) => {
  ...
  try {
    ...
    if (!targetUserId || !newUsername) return new Response(JSON.stringify({ error: 'Missing params' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })

    if (!/^[a-zA-Z0-9_]{3,30}$/.test(newUsername)) {
      return new Response(JSON.stringify({ error: 'Логин: 3-30 символов, только латиница, цифры, _' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (existing) return new Response(JSON.stringify({ error: 'Этот логин уже занят' }), { status: 409, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
