-- P1: серверная валидация наличия при оформлении заявки с маркета.
-- Раньше submit_marketplace_order не проверял склад → можно было заказать проданную/
-- отсутствующую позицию или больше остатка (оверселл). Добавлена проверка: товар должен
-- быть status='available' и запрошенное кол-во ≤ quantity. Нарушение → откат всей заявки.
-- (Полноценный резерв с reserved_quantity и освобождением по всем статусам — отдельный этап;
--  здесь — безопасная проверка без «зависающего» резерва.)
...
begin
  ...
  for it in select * from jsonb_array_elements(p_items) loop
    ...
    -- Проверка наличия (если позиция связана со складом)
