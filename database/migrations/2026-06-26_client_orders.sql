-- Кабинет клиента разборок: привязка заказов с маркета к залогиненному покупателю.
-- Гостевые заказы (без входа) остаются с user_id=null; залогиненные получают user_id=auth.uid()
-- → покупатель видит свои заказы в личном кабинете («Мои заказы» рядом с «Мои авто»).

alter table public.marketplace_orders
  add column if not exists user_id uuid references auth.users(id) on delete set null;
create index if not exists idx_marketplace_orders_user on public.marketplace_orders(user_id);

-- RPC submit_marketplace_order: добавлен user_id := auth.uid() в insert (остальное без изменений).
create or replace function public.submit_marketplace_order(p_company_id uuid, p_buyer_phone text, p_buyer_name text, p_comment text, p_items jsonb)
 returns uuid
 language plpgsql
 security definer
...
-- RLS: покупатель видит СВОИ заказы и их позиции (в дополнение к чтению разборкой/админом).
drop policy if exists market_orders_select_own on public.marketplace_orders;
create policy market_orders_select_own on public.marketplace_orders
  for select to authenticated using (user_id = auth.uid());

drop policy if exists market_order_items_select_own on public.marketplace_order_items;
create policy market_order_items_select_own on public.marketplace_order_items
